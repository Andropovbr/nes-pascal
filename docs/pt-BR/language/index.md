# Guia da Linguagem

[English](../../language/index.md) | Português (Brasil)

Este guia descreve apenas o subconjunto atualmente implementado pelo NES Pascal.
A sintaxe é inspirada em Pascal, mas a linguagem não foi projetada para ser compatível
com Pascal ou Free Pascal.

- [Estrutura do programa](program-structure.md)
- [Identificadores e literais](identifiers-and-literals.md)
- [Tipos embutidos](types.md)
- [Constantes e variáveis](constants-and-variables.md)
- [Atribuições](assignments.md)
- [Expressões](expressions.md)
- [Estruturas condicionais](conditionals.md)
- [Laços de repetição](loops.md)
- [Incremento e decremento](increment-and-decrement.md)
- [Procedimentos](procedures.md)

Os comandos embutidos implementados estão documentados separadamente em
[Runtime do NES](../runtime/index.md). Consulte
[Recursos não suportados](../reference/unsupported-features.md) para limitações
explícitas da linguagem e da plataforma.
