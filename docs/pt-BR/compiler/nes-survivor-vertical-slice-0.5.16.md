...
5. rode `python -m unittest tests.test_nes_survivor_vertical_slice -v` para
...
CHR final da ROM de referência. Nenhum tile foi redesenhado ou movido. O runtime
seleciona sprites em `$0000-$0FFF` e background em `$1000-$1FFF`, como a
referência.
...
O NES Pascal preserva a organização dos pattern tables, mas ainda não possui o
pequeno loader direto de texto da referência. Incluir a fila geral atual de
...
| --- | ---: |
| Código PRG | 5.908 B |
| PRG ocupado, incluindo vetores | 5.914 B |
| Header iNES | 16 B |
| CHR-ROM | 8.192 B |
| Linhas de Assembly gerado | 4.904 |
| Instruções geradas | 2.653 |
| Soma estática de ciclos-base | 8.099 |
...
Os 8.099 ciclos são a soma do estimador sobre o stream estático inteiro, não um
...
quadros de vídeo e observa 107 updates concluídos: aproximadamente 53,5 updates
por segundo numa linha do tempo NTSC de 60 Hz. Não há crash, freeze, opcode
...
| Stress cheio | 1.735/1.735 updates após baseline | 107/120 com 41 sprites |
...
| **ERGONOMICS/BOILERPLATE GAP** | Não há locais, inicialização integral de record ou `inc`/`dec` de campo indexado. | Usar variáveis de trabalho compartilhadas e reset/update campo a campo; contribui para as 663 linhas. | Sim | Tratar somente em trabalho de linguagem separado. |
...
| **CODEGEN/PERFORMANCE GAP** | Estado genérico por metasprite e escalonamento repetido de arrays de records reduzem o update rate no pico. | Manter APIs genéricas e exigir a baseline medida de 107/120 updates; sem alterar o otimizador. | Sim | Preservar o workload para medir backend estruturado e footprint. |
...
- Não há variáveis locais nem inicialização do record inteiro. Variáveis de
  trabalho globais e reset campo a campo aumentam o exemplo para 663 linhas.
...
- A animação/renderização genérica por instância perde 13 de 120 updates no
...
toolchain e Mesen. A linha de base local final é de 590 testes automatizados,
