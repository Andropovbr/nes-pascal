...
| `assets/arena_blank.nam` | NES Pascal integration fixture | `30a0c265f473bc034386a7d6c3f42b81ada1da6bccb03393602c2c240947896a` | Static 960-byte `$15` tile map plus 64 zeroed attributes |
...
5. generate `arena_blank.nam` as 960 `$15` bytes followed by 64 `$00` bytes;
6. run `python -m unittest tests.test_nes_survivor_vertical_slice -v` to verify
...
composition byte for byte. No tile was redrawn or relocated. Runtime inspection
corrected an earlier documentation assumption: during Playing, the
authoritative `PPUCTRL` value is `$80`. Bit 3 and bit 4 are both clear, so
sprites **and backgrounds both select pattern table `$0000-$0FFF`**. The font
remains stored in `$1000-$1FFF`, but this slice does not select or render it.
...
NES Pascal preserves the CHR organization, but it does not yet have the
reference's small direct state-screen text loader. Pulling in the current
...
### Arena background correction

The original 0.5.16 ROM showed one tile pattern repeated across the arena.
Mesen inspection of the unmodified ROM established the complete cause:

- Playing restored `PPUCTRL=$80` and `PPUMASK=$1E`; background pattern-table
  selection was therefore table 0, not table 1;
- no static background was configured, so RESET never initialized nametable 0;
- Mesen's representative nametable and attribute bytes were `$00`, although
  real NES power-on VRAM contents are not guaranteed;
- nametable value `$00` selected pattern-table-0 tile `$00`, whose frozen 16
  bytes are nonblank Soldier graphics;
- pattern-table-1 tile `$00` was blank, but that table was not selected.

The issue was not palette restoration or stale scroll state. It was the
combination of an uninitialized nametable, the actual table-0 selection, and a
nonblank tile `$00`.

The focused fix adds `arena_blank.nam` and calls the existing
`nes.load_background()` initialization command once before `nes.run`. All 960
tile entries are `$15`, the first blank tile after the populated table-0 art,
and all 64 attribute bytes are `$00`. The uploader runs while rendering is
disabled, so power-on VRAM contents cannot affect the result. Damage feedback
changes only the universal palette color; after the exact ten-update countdown,
black returns over the unchanged blank nametable. Restart reuses that same
deterministic PPU state.

This adds no persistent RAM, Zero Page, runtime feature, queue, or shadow. It
adds 1,083 PRG bytes: 1,024 bytes of static nametable data and 59 bytes of
one-time upload code. The emitted initialization adds 25 instructions and 76
static base cycles; it does not run in the gameplay loop.
...
| --- | ---: |
| PRG code | 6,991 B |
| PRG occupied, including vectors | 6,997 B |
| iNES header | 16 B |
| CHR-ROM | 8,192 B |
| Generated Assembly lines | 5,003 |
| Generated instructions | 2,678 |
| Static base-cycle sum across emitted instructions | 8,175 |
...
The 8,175-cycle value is the benchmark estimator's sum over the emitted static
...
completed gameplay updates before the arena follow-up and 106 afterward. The
post-fix result is approximately 53 updates/second on an NTSC 60 Hz timeline.
The ROM does not crash, freeze, execute invalid code, or
...
| Full stress | 1,735/1,735 gameplay updates after baseline | 106/120 updates at 41 sprites |
...
| **ERGONOMICS/BOILERPLATE GAP** | No locals, whole-record initialization, or indexed-field `inc`/`dec`. | Use shared work variables and reset/update every field explicitly; contributes to the 664-line source. | Yes | Address only through separately scoped language work. |
...
| **CODEGEN/PERFORMANCE GAP** | Generic state per metasprite and repeated array-of-record scaling reduce peak update rate. | Retain generic APIs and enforce the 100/120 regression floor around the measured 106/120 result; no optimizer changes. | Yes | Preserve the workload for structured-backend and runtime-footprint measurement. |
...
- The language has no local variables or whole-record initialization. Shared
  work variables and explicit field resets make the 664-line example longer
...
- General per-instance metasprite animation/rendering misses 14 of 120 updates
...
Mesen regression coverage. The final local baseline is 591 automated tests,
