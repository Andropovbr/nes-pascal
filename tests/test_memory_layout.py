...
class MemoryLayoutTests(unittest.TestCase):
    ...
    def test_allocations_never_use_zero_page_stack_oam_or_mirrors(self) -> None:
        layout = build_memory_layout(
            resolved_program(small_program(("First", "Second")))
        )
        allocations = (*layout.temporary_symbols, *layout.user_symbols)
    ...
    def test_exact_final_byte_succeeds_and_one_more_fails(self) -> None:
        settings = MemoryLayoutSettings(runtime_data_size=1263)
    ...
    def test_multiple_variables_collectively_exhaust_user_ram(self) -> None:
        ...
        with self.assertRaises(CompilerError) as context:
            build_memory_layout(
                ...
                MemoryLayoutSettings(runtime_data_size=1263),
                source=source,
                filename=str(path),
            )
        self.assertEqual(context.exception.code, DiagnosticCode.USER_RAM_EXHAUSTED)
    ...
    def test_linker_configuration_is_derived_from_the_memory_model(self) -> None:
        ...
        self.assertIn("ZEROPAGE: start = $0000, size = $0100", config)
        ...
        self.assertIn("TEMP:    start = $0300, size = $0010", config)
        self.assertIn("USER:    start = $0310, size = $04F0", config)
        ...
        self.assertIn("TEMP:    start = $0304, size = $0008", custom_config)
        self.assertIn("USER:    start = $030C, size = $04F4", custom_config)
