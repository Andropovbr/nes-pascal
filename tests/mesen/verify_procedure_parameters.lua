...
local function validateProcedureParameters()
    ...
    if not expectByte(0x0300, 0x06, "Counter") then
        return
    elseif not expectByte(0x0301, 0x01, "Enabled") then
        return
    elseif not expectByte(0x0302, 0x21, "BackgroundColor") then
        return
    elseif not expectByte(0x0304, 0x04, "Initialize.Step") then
        return
    elseif not expectByte(0x0306, 0x04, "ApplyStep.Amount") then
        return
    elseif not expectByte(0x0308, 0x01, "SelectColor.Matches") then
        return
    end
